...
{
  hb_retnint(static_cast<intptr_t>(s_CreateDlgTemplate(hb_param(1, HB_IT_OBJECT), hb_parni(2), hb_parni(3), hb_parni(4),
...
{
  s_ReleaseDlgTemplate((LPDLGTEMPLATE)static_cast<intptr_t>(hb_parnint(1)));
}
...
HB_FUNC(HWG__CREATEPROPERTYSHEETPAGE)
{
  ...
  if (hb_itemGetNI(GetObjectVar(pObj, "TYPE")) == WND_DLG_RESOURCE) {
    ...
    pdlgtemplate = (LPDLGTEMPLATE)static_cast<intptr_t>(hb_parnl(2));
...
HB_FUNC(HWG__PROPERTYSHEET)
{
  ...
  for (i = 0; i < nPages; i++) {
    psp[i] = (HPROPSHEETPAGE)static_cast<intptr_t>(hb_arrayGetNL(pArr, i + 1));
  }
...
HB_FUNC(HWG_CREATEDLGINDIRECT)
{
  ...
  if (hb_pcount() > 7 && !HB_ISNIL(8)) {
    pdlgtemplate = (LPDLGTEMPLATE)static_cast<intptr_t>(hb_parnl(8));
  } else {
