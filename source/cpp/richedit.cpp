...
HB_FUNC(HWG_CREATERICHEDIT)
{
  ...
  hCtrl = CreateWindowEx(0, TEXT("RichEdit20A"), nullptr, WS_CHILD | WS_VISIBLE | hwg_par_DWORD(3), hwg_par_int(4),
...
HB_FUNC(HWG_LOADRICHEDIT)
{
  ...
  lpFileName = HB_PARSTR(2, &hFileName, &nSize);
  hFile =
      CreateFile(lpFileName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, nullptr);
  if (hFile == INVALID_HANDLE_VALUE) {
    hb_retni(0); // TODO: revisar valor de retorno
    return;
  }
